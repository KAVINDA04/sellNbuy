		    ...
		    <div class="reservations">
	            <table class="reservationTable">
	                <tr>
	                    ...
	                    <th class="tableNew">Cancel Request</th>
						...
						%><tr>
							...
							<td class="tableNew"><p id="<%=payment.getId() %>" style="cursor: pointer" onclick="clickRefund(this);"><%=refund %></p></td>
						  </tr>
						<%
							}
						%>
...
<script>
	...
	function clickCancel(id){
		...
		var ans = $.ajax({
			type : 'POST',
			url : 'http://localhost:8080/Ebuy/rest/payment/cancel',
			dataType : 'json',
		...
		ans.done(function(data){
			if(data['success']=="1"){
				swal("Success!", "Rejected Successfull!", "success");
				$('#cancelDiv').empty();
				$("#cancelDiv").load(location.href + " #cancelDiv");
			}else if(data['success']=="0"){
				swal({
		            title: "Error",
		            text: "Rejected Unuccessfull!",
		            icon: "warning",
		            dangerMode: true,
		        });
			}
	...
	function clickRefund(id){
		...
		var ans = $.ajax({
			type : 'POST',
			url : 'http://localhost:8080/Ebuy/rest/payment/refund',
			dataType : 'json',
			contentType : 'application/json',
			data : jsonfile
		});
		...
		ans.done(function(data){
			if(data['success']=="1"){
				swal("Success!", "Refunded Successfull!", "success");
				$('#cancelDiv').empty();
				$("#cancelDiv").load(location.href + " #cancelDiv");
			}else if(data['success']=="0"){
				swal({
		            title: "Error",
		            text: "Refunded Unuccessfull!",
		            icon: "warning",
		            dangerMode: true,
		        });
			}
